...
class Home extends Component {
  ...
  componentDidMount() {
    if(this.context.email) {
      axios.post(url + endpoint.profile, { email: this.context.email })
        .then((res) => {
          this.setState({ userStudentAssociation: res.data.studentAssociation });
        })
        .catch((err) => {
          console.log(err);
        });
      ...
      >
        ...
        >
         {this.context.loggedIn ? 
            <>
              <option value="">{language.selectStudentAssociation[this.context.language]}</option>
              <option value={this.state.userStudentAssociation}>{this.state.userStudentAssociation}</option>
            </>
          :
            <>
              <option value="">{language.selectStudentAssociation[this.context.language]}</option>
